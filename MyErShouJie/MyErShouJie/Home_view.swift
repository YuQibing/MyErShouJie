...
class Home_view: UICollectionViewCell {
    ...
    override init(frame: CGRect){
        ...
        priceLabel = UILabel(frame: CGRect(x: 5, y: titleLabel!.frame.maxY, width: (width-40)/2, height: 50))
        priceLabel?.numberOfLines = 0
        priceLabel?.font = UIFont.boldSystemFont(ofSize: 14.0)
        priceLabel?.textColor = UIColor.lightGray
        self.addSubview(priceLabel!)
        
        readLabel = UILabel(frame: CGRect(x: (width-30)/2/2, y: titleLabel!.frame.maxY, width: (width-40)/2, height: 20))
